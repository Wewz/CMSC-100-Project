...
const OrderListPending = ({transactions, setTransactions, setOrderToShow, setShowDetails, actionTaken, setActionTaken}) => {
  ...
  const fetchOrders = async () => {
        try {
            const response = await fetch(`http://localhost:3001/get-transaction-using-status/${0}`);
            if (!response.ok) {
